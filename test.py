...
def load_movie_data(file_name='movie_data.pkl'):
    with open(file_name, 'rb') as input:
        movie_data = json.load(input)
    return movie_data
...
def convert_sentence_to_index(sentence):
    ...
    for i in range(0, len(sent_arr)):
        if sent_arr[i] in w2i:
            sent_indx[i] = w2i[sent_arr[i]]
        else:
            sent_indx[i] = w2i['unknown']
    return sent_indx
...
movie_data = load_movie_data(file_name="test_data2.json")
count = 0
    ...
    with torch.no_grad():
        ...
        for movie in movie_data:
            ...
            if (count > 0):
                ...
                plot = movie['documents']['plot']
                review = movie['documents']['review']
                comments = churn_comments(movie['documents']['comments'])
                review = churn_review(review)
                plot_sent_indx_arr = convert_knowledge(plot)
                review_sent_indx_arr = convert_knowledge(review)
                comment_sent_indx_arr = convert_knowledge(comments)
                ...
                for input, target in zip(chats[0::2], chats[1::2]):

                    encoder_in = [convert2(input)]
                    decoder_ou = [convert2(seq) for seq in target]
                    ##encoder_in.extend(talk[0::1])
                    ##decoder_ou.extend(talk[1::2])
                    if (len(encoder_in) < batch_size):
                        diff = batch_size - len(encoder_in)
                        for k in range(diff):
                            encoder_in.append("<PAD>")
                            decoder_ou.append("<PAD>")
                    enc_lengths = []
                    dec_lengths = []
                    for ea in encoder_in:
                        enc_lengths.append(len(ea))
                    for da in decoder_ou:
                        dec_lengths.append(len(da))
                    kj = 0
                    for lo in dec_lengths:
                        if (lo > 40):
                            kj = 1
                    if (kj == 1):
                        kj = 0
                        continue
                    # input_sent, dec_sent_index, = preprocess(encoder_in, decoder_ou)
                    # enc_sent = [convert2(seq) for seq in encoder_in]
                    # dec_sent = [convert2(seq) for seq in decoder_ou]
                    enc_lengths = torch.tensor(enc_lengths).long().to(device)
                    dec_lengths = torch.tensor(dec_lengths).long().to(device)
                    input_sent = torch.tensor(encoder_in).long().to(device)
                    ...
                    output, coverage, current_attention = model.forward(input_sent, dec_sent_index,
                                                                            ...
                                                                            dec_lengths,(uniq_indxs,indx_dic))
                    """
                    output = model.forward(input_sent,decoder_ou,
                                                                        start_index,
                                                                        False, know_hidd,
                                                                        isRely,[],[],[],enc_lengths, [],[])
                    org_word_index = dec_sent_index.clone()
